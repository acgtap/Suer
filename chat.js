...
async function getRandomImg(keyWord, page = 1, type = 1) {
  ...
  switch (type) {
    case 1:
      url =
        "https://www.pkdoutu.com/search?keyword=" +
  ...
  Log.trace("get:" + url);
  await request2.get(url, function (err, response, body) {
    /*
      response 响应信息的集合
    */
    if (!err && response.statusCode == 200) {
      ...
      if (!iamges || iamges.length <= 0) {
        Log.error("This none img ,will handle by randomimg");
        let img = global.config.pic;
        img = tool.randomArray(img)(async () => {
          await bot.sendMessage({
            friend: fromQQ,
            message: new Message().addImageUrl(img),
          });
        })();
      ...
      Log.info("got img ,will send：" + iamges);
      bot.sendMessage({
        friend: fromQQ,
        message: new Message().addImageUrl(iamges),
      });
      return iamges;
    } else {
      Log.error("获取表情失败");
